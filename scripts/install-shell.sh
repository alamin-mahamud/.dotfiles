...
# Install Custom Neovim Configuration (idempotent)
install_custom_neovim_config() {
    ...
    if [[ -d "$HOME/.config/nvim" ]] && [[ -f "$HOME/.config/nvim/init.lua" ]]; then
        local current_config=$(head -2 "$HOME/.config/nvim/init.lua" 2>/dev/null | grep -c "config.lazy" || echo "0")
        if [[ "$current_config" -gt 0 ]]; then
            print_success "Custom Neovim configuration already installed"
            return 0
        fi
    fi
    ...
    if [[ -d "$HOME/.config/nvim" ]] && [[ ! -L "$HOME/.config/nvim" ]]; then
        ...
        mv "$HOME/.config/nvim" "$BACKUP_DIR/nvim.bak.$(date +%Y%m%d_%H%M%S)" 2>/dev/null || true
    fi

    # Remove existing symlink or directory
    rm -rf "$HOME/.config/nvim"

    # Download custom configuration from ishmum123/config repository
    print_status "Installing custom Neovim configuration from ishmum123/config..."
    
    # Create temp directory for download
    local temp_dir="/tmp/nvim-config-$(date +%s)"
    mkdir -p "$temp_dir"
    
    # Clone just the nvim directory
    git clone --quiet --depth 1 --filter=blob:none --sparse https://github.com/ishmum123/config.git "$temp_dir"
    cd "$temp_dir"
    git sparse-checkout set nvim
    
    # Copy nvim configuration to proper location
    mkdir -p "$HOME/.config"
    ...
    print_success "Custom Neovim configuration installed. Run 'nvim' to complete setup."
}
...
show_summary() {
    ...
    print_success "✓ Neovim installed with custom configuration"
    ...
    echo "  • ~/.config/nvim/ - Neovim custom configuration"
    echo "  • ~/.tmux.conf - Tmux config (Tokyo Night Moon theme)"
    echo
    ...
    echo "  4. Run 'nvim' to complete Neovim setup"
...
main() {
    ...
    install_shell_tools
    # Editor
    # Note: For desktop environments, install fonts and kitty terminal separately
    install_neovim
    install_custom_neovim_config
